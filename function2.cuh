...
void PolyMullong(uint64_t *a, uint64_t *trlwekey, uint64_t *product, Params128 &p128)
{
    ...
    // TwistFFTlong(a, p128, h_Comp_a);
    // TwistFFTlong(trlwekey, p128, h_Comp_trlwekey);
...
void trlweSymEnc64(uint64_t *vecmu, uint64_t *trlwekey, double stdev, Params128 &p128, uint64_t **c)
{
    ...
    // a[0] = 6652511352535247738;
    // a[1] = 16079919947462470551;
    // a[2] = 11638417236812866510;
    // a[3] = 952530476690113135;
    // a[4] = 6652511352535247738;
    // a[5] = 16079919947462470551;
    // a[6] = 11638417236812866510;
    // a[7] = 952530476690113135;
    for (int i = 0; i < p128.nbar; i++)
    {
        a[i] = g();     // 原来的随机生成
        // a[i] = dist(generator);
...
{   
    ...
    // trgswSymEnc(vecmu, trlwekey, p128, c);
    uint32_t cc[4][2][2] = {
        784832207, 2494857248, 2657219734, 3437857634,
        1609388475, 675202580, 3152673806, 467090910,
        711420553, 2233177365, 679378986, 1386605222,
        373741434, 495835955, 3860182687, 4233920040
    };
...
void Test_TRGSW64_case()
{
    ...
    uint64_t cc[8][2][2] = {4281253880797575168, 4720940334384275456, 4740289327773332480, 8985514411551944077,
    285072914448234752, 4575150417897147392, 16016667591908949623, 16301670137613006530,
    2301464913528097792, 10614329988139742208, 2765474654740332787, 5066939430829476780,
    8783176099965077504, 3298325125279717376, 3182621424366595920, 11965797524063238495,
    17320513398715238400, 9223372036854775808, 13210341871622861700, 12048082399609584574,
    14998301526248789504, 9223372036854775808, 14087166198234005115, 10638653282029065488,
    6006794815940102144, 10755764232697831424, 842092649974762070, 6848887328475911320,
    9223372036854775808, 18001981498065819648, 12524517363832703951, 6366989285789015515};
...
void Test_ExternalProduct64()
{
    ...
    // {
    //     cout << p128.twistlong[i].x << "+" << p128.twistlong[i].y << "j, ";
    // }
    // cout << endl;
